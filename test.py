...
class AWSVulnerabilityApp:
    ...
    def run_comprehensive_scan(self, selected_services):
        ...
        with st.spinner('🚀 **Launching Comprehensive Security Assessment...**'):
            try:
                ...
                steps = [
                    ...
                    "☸️ Analyzing EKS clusters...",
                    "λ Inspecting Lambda functions...",
                    "🪣 Checking S3 buckets...",
                ...
                status_text.text("✅ **Security Assessment Complete!**")
    ...
    def _display_security_overview(self):
        ...
        with col1:
            if st.session_state.last_scan_time:
                ...
                st.info(f"**Last Assessment:** {last_scan} | **Mode:** {'🔬 Demo' if not self.aws_clients else '🚀 Live'}")
            else:
                st.info("**Ready for security assessment** | **Mode:** 🔬 Demo")
    ...
    def display_welcome(self):
        ...
        with col1:
            ...
            <div style='text-align: center; padding: 20px;'>
                <h2 style='color: #333; margin-bottom: 10px;'>🔍 Comprehensive Cloud Security</h2>
            ...
            st.subheader("🎯 Protected Services")
            services = [
                ("🖥️", "EC2", "Compute Security", "Instances, AMIs, Security Groups"),
                ("☸️", "EKS", "Kubernetes Security", "Clusters, Pods, RBAC"),
                ("λ", "Lambda", "Serverless Security", "Functions, Permissions, Runtime"),
                ("🪣", "S3", "Storage Security", "Buckets, Encryption, Access"),
                ("👤", "IAM", "Access Security", "Users, Roles, Policies"),
                ("🌐", "VPC", "Network Security", "Subnets, NACLs, Flow Logs")
            ]
        ...
        with col2:
            ...
            <div style='background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); 
                ...
                <h3 style='color: white; margin-bottom: 15px;'>🤖 AI-Powered Security</h3>
    ...
    def _display_dashboard_tab(self, resources, vulnerabilities):
        ...
        with col1:
            self._create_metric_card("📈", "Resources", len(resources), "#667eea")
        with col2:
            self._create_metric_card("🚨", "Vulnerabilities", len(vulnerabilities), "#ff6b6b")
        with col3:
            critical = len([v for v in vulnerabilities if v['severity'] == 'CRITICAL'])
            self._create_metric_card("🔥", "Critical", critical, "#ff4757")
        with col4:
            self._create_metric_card("🛡️", "Security Score", f"{st.session_state.security_score}/100", "#2ed573")
        ...
        insights = [
            "**🔍 Pattern Detection:** Multiple EC2 instances with public IPs detected. Consider implementing a bastion host pattern.",
            "**⚡ Quick Wins:** 3 critical vulnerabilities can be remediated automatically with minimal impact.",
            "**📈 Trend Analysis:** Security score improved by 12% compared to last assessment.",
            "**🛡️ Recommendations:** Enable AWS Security Hub for continuous monitoring and automated compliance checks.",
            "**💰 Cost Impact:** Remediating these issues could prevent potential losses of ~$50K annually.",
            "**⏱️ Time to Remediate:** Estimated 2.5 hours for all critical and high severity issues."
        ]
    ...
    def _display_vulnerabilities_tab(self, vulnerabilities):
        ...
        if not vulnerabilities:
            st.success("🎉 No vulnerabilities found! Your AWS environment is secure.")
            return
        ...
        for idx, vuln in enumerate(filtered_vulns):
            ...
            <div class="vulnerability-card" style="border-left-color: {border_color};">
                <div style="display: flex; justify-content: between; align-items: start;">
                    <div style="flex: 1;">
                        <h3 style="margin: 0 0 10px 0; color: #333;">{vuln['title']}</h3>
                        <div style="display: flex; gap: 15px; margin-bottom: 10px; flex-wrap: wrap;">
                            <span class="severity-{vuln['severity'].lower()}">{vuln['severity']}</span>
                            <span style="background: #e9ecef; padding: 4px 12px; border-radius: 15px; font-size: 0.9rem;">
                                {vuln['service']} - {vuln['resource_name']}
                            </span>
                            <span style="background: #ffeaa7; padding: 4px 12px; border-radius: 15px; font-size: 0.9rem;">
                ...
                <div style="margin: 15px 0;">
                    <p style="margin: 0; color: #666; line-height: 1.5;">{vuln['description']}</p>
                </div>
                
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px; margin: 15px 0;">
                    <div>
                        <strong>🔄 Remediation:</strong>
                        <p style="margin: 5px 0; color: #28a745;">{vuln['remediation']}</p>
                    </div>
                    <div>
                        <strong>📋 Category:</strong>
                        <p style="margin: 5px 0;">{vuln['category']}</p>
                        <strong>🎯 Exploitability:</strong>
                        <p style="margin: 5px 0;">{vuln.get('exploitability', 'Medium')}</p>
                    </div>
                </div>
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px;">
                    <div>
                        ...
                        <div class="progress-bar">
                            <div class="progress-fill" style="width: {vuln.get('risk_score', 50)}%; background: {border_color};"></div>
                        </div>
                        <small>{vuln.get('risk_score', 50)}/100</small>
                    </div>
                    <div>
    ...
    def _display_ai_analysis_tab(self, vulnerabilities):
        ...
        if not selected_vulns:
            ...
            if quick_vuln and st.button("🤖 Analyze This Vulnerability"):
    ...
    def _display_remediation_tab(self, vulnerabilities):
        ...
        st.header("⚡ Remediation Hub")
        
        selected_vulns = st.session_state.selected_vulnerabilities
        
        if not selected_vulns:
            st.info("🎯 Select vulnerabilities from the Vulnerabilities tab to begin remediation planning.")
            return
        ...
        for idx, vuln in enumerate(selected_vulns):
            ...
            with col1:
                ...
                **{vuln['title']}** 
                ...
                """, unsafe_allow_html=True)
            with col2:
                if st.button("📋 Plan", key=f"plan_{idx}"):
        ...
        with col1:
            if st.button("📋 Generate Plan", use_container_width=True):
                ...
                st.success("✅ Comprehensive remediation plan generated!")
        ...
        if st.session_state.remediation_plan:
            st.subheader("📄 Generated Remediation Plan")
    ...
    def _execute_remediation(self, vulnerabilities):
        ...
        for i, step in enumerate(steps):
            ...
            with results_container:
                if i == 3:  # During configuration application
                    for vuln in vulnerabilities:
                        st.success(f"✅ Configured: {vuln['title']}")
        
        status_text.text("🎉 Remediation completed successfully!")
        st.balloons()
        ...
        with results_container:
            ...
            for vuln in vulnerabilities:
                st.write(f"• {vuln['title']} - ✅ Resolved")
